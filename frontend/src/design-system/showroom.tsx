...
const navSections = [
  ["FOUNDATIONS", [["overview", "Overview"], ["brand", "Brand & color"], ["type", "Typography"], ["space", "Spacing & shape"], ["motion", "Motion & icons"]]],
  ...
  ["PATTERNS", [["navigation", "Navigation"], ["overlays", "Overlays"], ["composition", "Page patterns"], ["responsive", "Responsive examples"], ["accessibility", "Accessibility"]]],
] as const;
      ...
      <div className="ff-ds-showroom">
        <header className="ff-ds-showroom__header">
          <div className="ff-ds-showroom__brand">
            ...
            <div><strong>FoodFighter</strong><span>Design System V4 · Fire &amp; Flavor</span></div>
          </div>
        ...
        <div className="ff-ds-showroom__layout">
          ...
          <main className="ff-ds-showroom__main">
            <section id="overview" className="ff-ds-showroom__intro">
              <div>
                <span className="ff-ds-eyebrow">V4 visual exploration</span>
                <h1>Design System Reference</h1>
                <p>FoodFighter V4 is a food-first library for a confident social product. The system is isolated to this showroom until the owner approves a migration path.</p>
              </div>
            ...
            <Section id="type" kicker="02 / Foundation" title="Expressive headlines. Clear UI." description="Poppins and Noto Sans Thai carry the task. A quiet editorial serif is reserved for food, winner, and hero moments.">
              <div className="ff-ds-showroom__panel"><div className="ff-ds-showroom__stack">{typographyScale.map((item, index) => <div key={item.name} className="ff-ds-showroom__type-row"><div className="ff-ds-showroom__type-meta"><strong>{item.name}</strong><br />{item.token}<br />{item.size} · {item.line}</div><div className={index < 2 ? "ff-ds-showroom__type-sample ff-ds-display" : "ff-ds-showroom__type-sample"} style={{ fontSize: index === 0 ? "clamp(2.8rem, 8vw, 6rem)" : index === 1 ? "clamp(2.2rem, 5vw, 4rem)" : undefined, lineHeight: item.line, fontWeight: Number(item.weight) }}>{index === 0 ? "Food, with a little fight." : index === 1 ? "กินอะไรดี? Let’s decide." : index === 2 ? "Build a better group meal" : index === 3 ? "Every voice gets a seat at the table" : index === 4 ? "Tom Yum · ต้มยำ" : index === 5 ? "Recommended for your room" : index === 6 ? "A warm, useful system for real food moments." : index === 7 ? "Choose a meal, compare the room, keep moving." : index === 8 ? "Supporting copy · 1,280 ฿" : index === 9 ? "ROOM STATUS · สถานะห้อง" : index === 10 ? "4 members · 2 paid" : "Updated just now · พร้อมแล้ว"}</div></div>)}</div></div>
            </Section>

            <Section id="space" kicker="03 / Foundation" title="Rhythm before decoration." description="Spacing, shape, and elevation are deliberate so food-forward surfaces stay useful instead of becoming theme wallpaper.">
            ...
            <Section id="motion" kicker="04 / Foundation" title="Tactile, calm, purposeful." description="Fast feedback is frequent. Emphasis is reserved for the moments that move a group forward. Use the control above to inspect reduced motion.">
            ...
            <Section id="actions" kicker="05 / Components" title="Actions that state their intent." description="Primary, secondary, tertiary, ghost, and danger variants share the same focus and press contract. Loading never changes button geometry.">
            ...
            <Section id="forms" kicker="06 / Components" title="Forms with enough room to breathe." description="Native controls keep semantics and keyboard behavior straightforward. Labels, helper text, errors, and success messages share an explicit API.">
            ...
            <Section id="navigation" kicker="11 / Patterns" title="Navigation that knows its context." description="The showroom demonstrates the future treatment only. It does not replace the current production ResponsiveNavigation or shell.">
            ...
            <Section id="overlays" kicker="12 / Patterns" title="Overlays keep the task in reach." description="Dialog, sheet, menu, and tooltip-like hints support secondary actions without hiding the primary path.">
              <div className="ff-ds-showroom__panel"><div className="ff-ds-showroom__row"><Button onClick={() => setDialogOpen(true)}>Open confirmation dialog</Button><Button variant="secondary" onClick={() => setSheetOpen(true)}>Open mobile sheet</Button><Menu label="Account menu" items={[{ label: "Profile" }, { label: "Food Profile" }, { label: "Log out", destructive: true }]} /><Tooltip label="Hints never contain the only critical action."><CircleDollarSign aria-hidden="true" /></Tooltip><HelpHint>Tooltips explain; they never hold the only action.</HelpHint></div></div><Dialog open={dialogOpen} title="Start this FoodFight?" description="A dialog should confirm a meaningful action and keep the next step explicit." onClose={() => setDialogOpen(false)} footer={<><Button variant="ghost" onClick={() => setDialogOpen(false)}>Cancel</Button><Button onClick={() => setDialogOpen(false)}>Start FoodFight</Button></>}><p>Four members are in the room. The group will move to Meal Preference.</p></Dialog><Sheet open={sheetOpen} title="Room actions" onClose={() => setSheetOpen(false)}><div className="ff-ds-stack"><ListItem title="Share room" description="Copy a local invite affordance" action={<Button size="sm">Share</Button>} /><ListItem title="Leave room" description="Destructive actions remain separated" action={<Button size="sm" variant="danger">Leave</Button>} /></div></Sheet></Section>

            <Section id="responsive" kicker="13 / QA" title="The same system at 390, 768, and 1440." description="The layout expands its composition, not just its width. These frames make media, typography, and action density reviewable at the target breakpoints.">
            ...
            <Section id="composition" kicker="14 / Patterns" title="Compositions stay explicit." description="Small page-level patterns keep context, media, and action hierarchy reusable without creating an all-purpose smart component.">
...
<MediaCard ratio="4:3" mediaSrc={image} metadata={<Badge />} />`}</CodeNote>
            ...
            <Section id="accessibility" kicker="15 / Contract" title="Accessible by default, expressive by choice." description="A practical review checklist for components that will later travel into product pages.">
...
function MediaFrameForShowroom({ src, label, ratio = "4:3", state = "auto" }: { src?: string; label: string; ratio?: "16:9" | "4:3" | "1:1" | "3:4"; state?: "auto" | "loading" | "missing" }) {
  return <MediaFrame src={src} alt={label} ratio={ratio} state={state} label={label} gradientProtection />;
}
